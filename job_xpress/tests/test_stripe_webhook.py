...
def sample_invoice_event():
    ...
    return {
        ...
        "data": {"object": {"customer": "cus_test123", "amount_paid": 999}},
    }
...
class TestStripeIdempotence:
    ...
    @pytest.mark.asyncio
    async def test_is_event_processed_returns_false_for_new_event(
    ...
    @pytest.mark.asyncio
    async def test_is_event_processed_returns_true_for_existing_event(
        self, mock_db_service
    ):
    ...
    @pytest.mark.asyncio
    async def test_is_event_processed_handles_db_error_gracefully(
        self, mock_db_service
    ):
        ...
        mock_db_service.admin_client.table.return_value.select.return_value.eq.return_value.limit.return_value.execute.side_effect = Exception(
            "DB Error"
        )
    ...
    async def test_mark_event_processed_inserts_record(self, mock_db_service):
        ...
        mock_db_service.admin_client.table.return_value.insert.return_value.execute = (
            mock_insert
        )
        ...
        call_args = mock_db_service.admin_client.table.return_value.insert.call_args[0][
            0
        ]
...
class TestStripeWebhookHandler:
    ...
    @pytest.mark.asyncio
    async def test_webhook_skips_already_processed_event(
        self, mock_db_service, sample_checkout_event
    ):
        ...
        mock_request.body = AsyncMock(
            return_value=json.dumps(sample_checkout_event).encode()
        )
    ...
    @pytest.mark.asyncio
    async def test_duplicate_event_does_not_double_credits(
        self, mock_db_service, sample_checkout_event
    ):
        """
        ...
        """
        from api.stripe_webhook import stripe_webhook, is_event_processed
        from fastapi import Request
        import json
        ...
        with patch("api.stripe_webhook.is_event_processed", mock_is_processed):
            with patch("api.stripe_webhook.mark_event_processed", mock_mark_processed):
                with patch(
                    "api.stripe_webhook.find_user_by_email",
                    AsyncMock(return_value="user_123"),
                ):
                    with patch(
                        "api.stripe_webhook.upgrade_user_subscription",
                        ...
                        mock_request.json = AsyncMock(
                            return_value=sample_checkout_event
                        )
...
class TestStripeWebhookEdgeCases:
    ...
    async def test_mark_processed_handles_insert_error(self, mock_db_service):
        ...
        mock_db_service.admin_client.table.return_value.insert.return_value.execute.side_effect = Exception(
            "Insert failed"
        )

        # Ne doit pas lever d'exception
        await mark_event_processed(event_id="evt_test", event_type="test", payload={})
