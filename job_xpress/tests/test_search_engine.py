...
class TestJobOfferModel:
    ...
    def test_job_offer_default_values(self):
        ...
        job = JobOffer(
            title="Test", company="Corp", description="Desc", url="https://example.com"
        )
...
class TestSearchStrategies:
    ...
    async def test_jsearch_strategy_builds_query(self, search_engine):
        ...
        with patch.object(
            search_engine, "_call_jsearch_api", new_callable=AsyncMock
        ) as mock_call:
    ...
    async def test_parallel_search_execution(self, search_engine):
        ...
        with patch.object(
            search_engine, "_search_jsearch_strategy", new_callable=AsyncMock
        ) as mock_jsearch:
            with patch.object(
                search_engine, "_search_active_jobs_db", new_callable=AsyncMock
            ) as mock_active:
                with patch.object(
                    search_engine,
                    "_enrich_jobs_with_full_content",
