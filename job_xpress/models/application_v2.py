...
class JobFilters(BaseModel):
    ...
    min_salary: Optional[int] = Field(
        None, ge=0, description="Salaire minimum en euros"
    )
    remote_only: bool = Field(False, description="Filtrer uniquement les offres remote")
    exclude_agencies: bool = Field(
        True, description="Exclure les cabinets de recrutement"
    )
    max_days_old: int = Field(
        14, ge=1, le=90, description="Ancienneté max des offres en jours"
    )
...
class SearchStartRequest(BaseModel):
    ...
    job_title: str = Field(
        ..., min_length=2, max_length=200, description="Intitulé du poste recherché"
    )
    location: str = Field(
        "France", max_length=100, description="Localisation souhaitée"
    )
    contract_type: str = Field("CDI", description="Type de contrat")
    work_type: str = Field(
        "Tous", description="Présentiel, Hybride, Full Remote ou Tous"
    )
    experience_level: str = Field(
        "Non spécifié", description="Junior, Confirmé ou Senior"
    )
    filters: Optional[JobFilters] = Field(
        None, description="Filtres avancés optionnels"
    )
    ...
    candidate_email: Optional[str] = Field(
        None, max_length=255, description="Email du candidat"
    )
...
class AdviceRequest(BaseModel):
    ...
    include_company_research: bool = Field(
        True, description="Inclure une analyse de l'entreprise"
    )
