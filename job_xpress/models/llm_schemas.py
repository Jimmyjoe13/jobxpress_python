...
class LLMScoreResponse(BaseModel):
    ...
    score_technical: int = Field(
        ge=0, le=100, default=0, description="Score technique (hard skills)"
    )
    score_structural: int = Field(
        ge=0, le=100, default=0, description="Score structurel (contrat/remote)"
    )
    score_experience: int = Field(
        ge=0, le=100, default=0, description="Score expérience (niveau)"
    )
    is_school_scheme: bool = Field(
        default=False, description="True si c'est une école/CFA"
    )
    ...
    @field_validator(
        "score_technical", "score_structural", "score_experience", mode="before"
    )
    @classmethod
    ...
    def calculate_weighted_score(
        self, w_tech: float = 0.4, w_struct: float = 0.3, w_exp: float = 0.3
    ) -> int:
        """
...
class LLMCoverLetterResponse(BaseModel):
    ...
    text_content: Optional[str] = Field(
        default=None, description="Version texte brut (optionnel)"
    )
