...
class UserSettingsBase(BaseModel):
    ...
    # Notifications
    email_candidatures: bool = Field(
        True, description="Recevoir un email à chaque candidature envoyée"
    )
    email_new_offers: bool = Field(
        True, description="Recevoir des alertes pour les nouvelles offres"
    )
    email_newsletter: bool = Field(
        False, description="Recevoir la newsletter hebdomadaire"
    )
    push_notifications: bool = Field(
        True, description="Activer les notifications push navigateur"
    )
    ...
    def validate_timezone(cls, v):
        ...
        allowed = ["Europe/Paris", "Europe/London", "America/New_York", "Asia/Tokyo"]
        if v not in allowed:
            return "Europe/Paris"
        return v
