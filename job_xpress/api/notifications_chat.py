...
async def mark_notification_read(
    ...
    client.table("notifications").update(
        {"read": True, "updated_at": datetime.now(timezone.utc).isoformat()}
...
async def accept_jobyjoba_offer(
    ...
    if notif["type"] != "offer_jobyjoba":
        raise HTTPException(
            status_code=400, detail="Cette notification n'est pas une offre JobyJoba"
        )
    ...
    app_result = (
        admin_client.table("applications_v2")
        .select("*")
    ...
    logger.info(
        f"🎉 Session JobyJoba créée: {session_id[:8]} pour {user_id[:8]} (plan {user_plan}, {max_messages} msgs {limit_info})"
    )
...
async def send_chat_message(
    ...
    if not session_result.data or len(session_result.data) == 0:
        raise HTTPException(
            status_code=404,
            detail="Session de chat non trouvée. Acceptez d'abord l'offre JobyJoba.",
        )
    ...
    if session["status"] != "active":
        raise HTTPException(
            status_code=400, detail="Cette session de chat est terminée."
        )
    ...
    app_result = (
        admin_client.table("applications_v2")
        .select("*")
        .eq("id", chat_request.application_id)
        .limit(1)
        .execute()
    )
