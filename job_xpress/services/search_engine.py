    ...
    if synonyms_path.exists():
        try:
            with open(synonyms_path, "r", encoding="utf-8") as f:
                ...
                logger.info(
                    f"📚 {len(synonyms)} catégories de synonymes chargées depuis {synonyms_path.name}"
                )
                return synonyms
...
class SearchEngine:
    ...
    async def find_jobs(
        ...
        logger.info(
            f"🔎 Recherche: {candidate.job_title} ({candidate.contract_type}) à {candidate.location}"
        )
        ...
        logger.info(
            f"📊 Bilan: {len(jobs_jsearch)} JSearch | {len(jobs_active)} Active Jobs"
        )
    ...
    # ==========================================
    async def _search_jsearch_strategy(
        ...
        if jsearch_type:
            ...
            if jobs:
                logger.info(f"✅ JSearch: {len(jobs)} offres (Strict)")
        ...
        if not jobs:
            ...
            if jobs:
                logger.info(f"✅ JSearch: {len(jobs)} offres (Sauvetage)")
            else:
                logger.error("❌ Échec total JSearch")
    ...
    # ==========================================
    async def _search_active_jobs_db(
        self, candidate: CandidateProfile
    ) -> List[JobOffer]:
        if not settings.RAPIDAPI_KEY:
            return []
        ...
        for title in final_titles:
            ...
            try:
                ...
                logger.debug(
                    f"Active Jobs HTTP error for {title}: {e.response.status_code}"
                )
            except Exception as e:
    ...
    # DEEP FETCHING
    # ==========================================
    async def _enrich_jobs_with_full_content(
    ...
    async def _fetch_single_url(self, offer: JobOffer) -> JobOffer:
        if not offer.url or not offer.url.startswith("http"):
            return offer
        try:
