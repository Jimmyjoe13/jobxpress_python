...
class RedisCache:
    ...
    def _connect(self):
        ...
        try:
            ...
            logger.warning(
                "⚠️ Package 'redis' non installé. Exécutez: pip install redis"
            )
            self._available = False
    ...
    def cache_search_results(
        self, job_title: str, location: str, filters: dict, results: list
    ) -> bool:
        """
        ...
        return self.set(
            key=cache_key, value=results, ttl=self.TTL_SEARCH, prefix=self.PREFIX_SEARCH
        )

    def get_cached_search(
        self, job_title: str, location: str, filters: dict
    ) -> Optional[list]:
        """
    ...
    def get_cached_credits(self, user_id: str) -> Optional[dict]:
        ...
        return self.get(key=f"credits:{user_id}", prefix=self.PREFIX_USER)

    def invalidate_user_credits(self, user_id: str) -> bool:
        """Invalide le cache des crédits (après débit ou upgrade)."""
        return self.delete(key=f"credits:{user_id}", prefix=self.PREFIX_USER)
