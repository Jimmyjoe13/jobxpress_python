...
class SearchEngineV2:
    ...
    async def find_jobs_v2(
        ...
        cached_results = redis_cache.get_cached_search(
            job_title=candidate.job_title, location=candidate.location, filters=filters
        )
        ...
        if jobs_to_enrich:
            ...
            jobs_to_enrich = await self.base._enrich_jobs_with_full_content(
                jobs_to_enrich
            )
        ...
        if jobs_to_enrich and redis_cache.is_available:
            try:
                ...
                logger.info(
                    f"💾 {len(jobs_to_enrich)} offres mises en cache Redis (TTL: 1h)"
                )
            except Exception as e:
        ...
        try:
            async with httpx.AsyncClient() as client:
                resp = await serpapi_circuit.call(
                    resilient_get, client, self.URL_SERPAPI, params=params, timeout=15.0
                )
    ...
    def _deduplicate_fuzzy(self, jobs: List[JobOffer]) -> List[JobOffer]:
        ...
        seen_keys: Dict[
            str, Tuple[int, Optional[datetime]]
        ] = {}  # key -> (index, date)
            ...
            for key, (idx, existing_date) in list(seen_keys.items()):
                ...
                if similarity > 90:
                    ...
                    if job_date and existing_date and job_date > existing_date:
                        ...
                        logger.debug(
                            f"🔄 Doublon mis à jour: {job.title} ({job.source})"
                        )
    ...
    def _apply_smart_filters(
        self, jobs: List[JobOffer], filters: Dict[str, Any], candidate: CandidateProfile
    ) -> List[JobOffer]:
        """
    ...
    def _is_title_match(self, job_title: str, candidate_title: str) -> bool:
        ...
        return (
            candidate_lower in job_lower or fuzz.ratio(job_lower, candidate_lower) > 85
        )
    ...
    def _parse_date(self, date_str: Optional[str]) -> Optional[datetime]:
        ...
        # Formats standards
        formats = ["%Y-%m-%d", "%d/%m/%Y", "%Y-%m-%dT%H:%M:%S", "%Y-%m-%dT%H:%M:%SZ"]
